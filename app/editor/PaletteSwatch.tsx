 ...
 * 목록과 범례가 서로 다르게 보이면 같은 색이 무엇을 뜻하는지 읽을 수 없다.
 */
export function PaletteSwatch({ item, size = 20 }: PaletteSwatchProps) {
  const color = item.color ?? FALLBACK_COLOR;
  const style =
    item.role === "kind"
      ? { width: size, height: size, background: "#ffffff", border: `2px solid ${color}` }
      : { width: size, height: size, background: color, border: "1px solid #94a3b8" };

...
- **이벤트 로그(서버)** — 별도 프로세스 `scripts/live-logger.ts` 가 브로커의 `…/reader/+/event` 를 구독해 `.grid-projects/.live/events/YYYY-MM-DD.<표시>.jsonl` 로 하루 한 파일씩 쌓습니다(기본 30일 보관). 상시 서비스 데몬이 서버와 함께 띄우고 함께 멈춥니다. 개발 서버는 같은 코드를 프로세스 안에서 돌려 `…dev.jsonl` 에 적고, 조회할 때 두 파일의 겹친 이벤트는 하나로 합칩니다. 브로커 주소 · 보관 일수는 `.grid-projects/.live/logger.json` (`{"broker":"mqtt://127.0.0.1:1883","prefix":"rfid","retentionDays":30}`), 상태는 `/api/live/events/status`. 상시 서비스의 라우트는 Cloudflare 호환 런타임에서 돌아 `net` 이 없기 때문에 웹 서버 자신이 구독하지 않고 파일만 읽습니다.
  - **영속 세션** — 상시 서비스의 기록기(`main`)는 고정 clientId(`grid-live-log-main`) + clean session false 로 붙습니다. 기록기가 죽어 있는 동안 브로커가 QoS 1 이벤트를 세션 큐에 쌓아 두고(Mosquitto `max_queued_messages`, 이 배치는 50000), 다시 붙으면 한꺼번에 내려줍니다. 늦게 온 이벤트는 받은 시각이 아니라 **이벤트 자신의 시각** 자리에 기록되어(`loggedAt` 에 실제 받은 시각) 이력이 실제 있었던 시간에 나타납니다. 개발 서버(`dev`)는 여러 개가 동시에 뜰 수 있어 임시 세션이 기본이고, `logger.json` 의 `"persistent": true|false` · `"clientId"` 로 바꿉니다. 상태(`/api/live/events/status`)의 `sessionPresent` 가 true 면 이어받은 세션, `late` 가 늦게 받은 건수입니다.
  - **재발행 받기** — 기록기는 `…/reader/+/replay` 도 구독합니다. 감시 PC(RfidReaderMonitor)가 자기 CSV 에서 다시 낸 이벤트가 이 토픽으로 오며, 페이로드는 `…/event` 와 같습니다. 현황판(브라우저)은 이 토픽을 구독하지 않으므로 재발행이 "지금 상태" 를 흔들지 않고 이력에만 들어갑니다.
- **빠진 이력 요청** — 기록기가 죽어 있던 구간(브로커 큐로도 못 채운 것)은 오른쬽 `감시 PC` 목록 아래 **빠진 이력 요청**을 펼쳐 시작 · 끝 시각(이 브라우저 시간대)과 대상 PC(모든 PC 또는 하나)를 고르고 `요청` 을 누릅니다. 브라우저가 자기 MQTT 접속으로 `{prefix}/{site}/replay`(또는 `…/host/{PC}/replay`) 에 요청(`{"v":1,"type":"replay","from":…,"to":…,"requestId":…}`)을 내면, RfidReaderMonitor **0.5.0 이상**의 감시 PC 가 자기 CSV(`%LOCALAPPDATA%\RfidReaderMonitor\logs\events\events-yyyyMMdd.csv`)에서 그 구간을 읽어 `…/reader/{key}/replay` 로 다시 내고 `…/host/{PC}/replay-done` 으로 건수를 알립니다. 답은 폼 아래에 PC 별로 쌓이고, 이력 창을 다시 열면 그 구간이 채워져 있습니다(같은 이벤트는 id 로 한 번만). 한 요청은 31일 · PC 당 20,000건까지. 형식은 RfidReaderMonitor `docs/mqtt/replay.schema.json` · `replay-done.schema.json`.

...
│     ├─ InspectorPanel.tsx        오른쪽 선택 정보 · 칸 장비 테두리
│     ├─ PaperModal.tsx            인쇄 용지 규격 모달 (툴바 `용지 설정` 으로 연다)
│     ├─ PrintOptions.tsx          툴바 둘째 줄 — 인쇄 용지 · 용지 설정 · 경계선 보기 · 메모 본문 인쇄
...
│     ├─ PaperForm.tsx             인쇄 용지 규격 입력 (모달 안)
│     │

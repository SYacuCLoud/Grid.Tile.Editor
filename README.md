...
- **범례도 경계 안에 함께** 그려집니다. 인쇄물에는 도면 아래로 범례가 실리므로 장수 계산에도 범례 행이 포함됩니다. 범례 띠는 **격자 너비만큼만** 차지합니다 — 가로 용지에서 격자 오른쪽에 남는 자리는 메모가 위에서 아래까지 씁니다(좁은 격자는 범례 열이 줄어 띠가 조금 두꺼워집니다).
...
npm test            # 타입 검사 → 빌드 → 서버 렌더 테스트 → 로직 테스트 (3100 서비스에 영향 없음)
...
│  ├─ m/                           📱 모바일 도면 보기 (읽기 전용)
│  │  ├─ page.tsx                  /m 엔트리 · 휴대폰 viewport 설정
│  │  ├─ MobileViewer.tsx          목록 · 페이지 칩 · 범례/메모/칸 시트 · 자동 갱신
│  │  ├─ MobileCanvas.tsx          한 손가락 이동 · 두 손가락 확대 · 탭 · 두 번 두드림
│  │  └─ mobileView.ts             맞춤 배율 · 오프셋 한계 · 확대 기준점 · 칸 요약 (순수 함수)
│  └─ editor/
...
│     ├─ InspectorPanel.tsx        오른쪽 선택 정보 · 격자 크기 · 인쇄 용지
│     ├─ DevicePanel.tsx           장치 대장 목록 모달 — 검색 · 배치/해제 · 내보내기
│     ├─ DeviceForm.tsx            장치 한 대의 명세 편집 폼
│     ├─ CellNotePopover.tsx       우클릭 칸 정보 편집 상자 — 장치 등록·연결 포함
...
│     ├─ device.ts                 장치 대장 — 모델 · 검사 · 연결/해제 · S/N 동기화
│     ├─ deviceExport.ts           장치 대장 CSV · 마크다운 내보내기
...
| `grid_set_cell` | 한 칸에 팔레트 항목을 칠하고 식별자·메모를 붙인다 |
